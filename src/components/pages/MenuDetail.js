    ...
    return (
      <div className="m-3">
        ...
        >
          ...
          <CardBody className="row p-0">
            <Col md={1} className="bg-light rounded p-0 m-auto" style={{ padding: 'none' }}>
              <CardImg top width="100%" height="100%" src={isImgLink(MenuURL) ? MenuURL : "https://incucdep.com/wp-content/uploads/2019/03/mau-thiet-ke-menu-bang-phan2.jpg"} alt="MenuIMG" />
            </Col>
        ...
        <div className="container p-3">
          <h2 className="my-4">Dishes List <span><button className="rounded-pill float-right btn btn-success" onClick={() => { setIsAdding(true); console.log(isAdding); }}>
          ...
          <Popup open={isAdding} position="right center" onClose={() => { setIsAdding(false); }}>
            <div className="position-fixed top-50 start-50 translate-middle">
              <AddDishToMenu close={() => { setIsAdding(false); console.log(isAdding); }} handleAddDish={handleAddDish} HomeCookID={HomeCookID}></AddDishToMenu>
            </div>
          </Popup>
          <DishList dishes={Dishes} handleRemoveDish={handleRemoveDish}></DishList>
        </div>
